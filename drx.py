...
import random
import string
import sys # Auto-detect filename for ENCRYPT DECRYPT
import os
import re # Add this to the top imports 
import subprocess
from telebot import apihelper
from datetime import datetime, timedelta

# --- [ CONFIGURATION ] ---
# --- [ 1. KEEP CONFIG LOAD LOGIC HERE ] ---
# This part should run first
...
# Start the bot after loading the token
if not TOKEN:
...
CONFIG = {
    ...
    "cooldown": 60,  # Default cooldown 60 seconds (You can customize this using /customize)
    "feedback_group_id": -1003221756039, # Put your group ID here
}
# --- [ EXPIRY CONFIGURATION ] ---
# Format: datetime(Year, Month, Day, Hour, Minute, Second)
SCRIPT_EXPIRY = datetime(2026, 6 , 15, 12, 30, 30) 

# --- [ GLOBAL EXPIRY CHECK ] ---
# This handler will trigger on every message and reply if the script is expired
@bot.message_handler(func=lambda message: is_script_expired())
def handle_expired_script(message):
    bot.reply_to(
        message, 
        "⚠️ <b>SCRIPT EXPIRED! CONTACT OWNER TO BUY PREMIUM SCRIPT @encryptdecrypt888</b>", 
        parse_mode="HTML")
...
# Placed in a list format so that random.choice works
COLOR_LIST = [
...
# --- [ 2. HELPER FUNCTION ] ---
# This gives the banner a random color
...
def send_welcome_pro(message):
    ...
    # Create 'all_users' list if it doesn't exist
    if 'all_users' not in db:
        db['all_users'] = []
    
    # Add new user to the list (with duplicate check)
...
def send_help(message):
    ...
    help_text = (
        "🛠️ <b>ENCRYPT DECRYPT 𝐇𝐄𝐋𝐏 𝐌𝐄𝐍𝐔</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
    )
...
def monitor_attack(attack_data, duration, chat_id):
    try:
        # 1. Execute binary in background
        ...
        # 2. WAIT FOR THE SPECIFIED DURATION (This ensures the attack does not end prematurely)
        time.sleep(int(duration))
        
    except Exception as e:
        print(f"Binary Error: {e}")

    # 3. Remove from active list and free slot
    if attack_data in active_attacks:
        ...
        finish_text = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🏁 <i>System is ready for the next command.</i>\n"
...
def handle_bgmi_fixed(message):
    ...
    if not re.match(ip_pattern, ip):
        bot.reply_to(message, "❌ <b>𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐈𝐏!</b> Please enter a valid IP address.", parse_mode="HTML")
        return

    if not port.isdigit() or not (1 <= int(port) <= 65535):
        bot.reply_to(message, "❌ <b>𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐏𝐨𝐫𝐭!</b> Range must be 1-65535.", parse_mode="HTML")
        return
    ...
    if len(active_attacks) >= CONFIG['max_slots']:
        bot.reply_to(message, "❌ <b>𝐀𝐥𝐥 𝐒𝐥𝐨𝐭𝐬 𝐅𝐮𝐥𝐥!</b> Please wait a moment.", parse_mode="HTML")
        return
    ...
    success_msg = (
        ...
        f"🕒 <b>𝐃𝐮𝐫𝐚𝐭𝐢𝐨𝐧:</b> <code>{duration}s</code>\n"
        f"⚡ <b>𝐌𝐞𝐭𝐡𝐨𝐝:</b> <code>VIP</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📸 <b>𝐒𝐄𝐍𝐃 𝐒𝐂𝐑𝐄𝐄𝐍𝐒𝐇𝐎𝐓 (𝐅𝐄𝐄𝐃𝐁𝐀𝐂𝐊)</b>\n"
        f"Make sure to send a screenshot after the attack completes!\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    # Register next step for photo feedback
...
def process_feedback_photo(message):
    ...
    if message.content_type == 'photo':
        try:
            ...
            bot.reply_to(message, "❤️ <b>𝐓𝐡𝐱𝐱 𝐟𝐨𝐫 𝐟𝐞𝐞𝐝𝐛𝐚𝐜𝐤!</b>\nYour screenshot has been uploaded to the group.", parse_mode="HTML")
        except Exception:
            bot.send_message(user_id, "❌ Error: Feedback upload failed. Please check bot permissions.")
    else:
        bot.reply_to(message, "⚠️ <b>𝐅𝐞𝐞𝐝𝐛𝐚𝐜𝐤 𝐂𝐚𝐧𝐜𝐞𝐥𝐥𝐞𝐝!</b>\nPhoto not found. Make sure to provide a screenshot after the next attack.")

# --- [ LIVE STATUS UPDATER ] ---
def update_status_live(message_obj):
    # This loop edits the message 5 times (every 3 seconds)
    for _ in range(5):
        time.sleep(3)
...
def handle_genkey_advance(message):
    ...
    if not (is_admin or is_reseller):
        return # No reply will be sent to normal users
    ...
    try:
        ...
        bot.reply_to(message, "❌ <b>Amount</b> must be a number!")
        return
...
def handle_redeem(message):
    ...
    if found_key:
        ...
        # Check remaining time for display
        ...
        bot.reply_to(message, "❌ <b>𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐨𝐫 𝐔𝐬𝐞𝐝 𝐊𝐞𝐲!</b>\nPlease enter a valid key or contact admin.", parse_mode="HTML")
...
def add_reseller_pro(message):
    ...
    # Initialize reseller data if it does not exist
    if 'resellers' not in db:
        db['resellers'] = {}

    # If the user is already a reseller, credits will be added, otherwise a new one will be created
    ...
    res = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ <i>Now this user can generate keys.</i>"
    )
    ...
    try:
        bot.send_message(target_id, (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"You have received <b>{credits_to_add} Credits</b>.\n"
            f"You are now an <b>Authorized Reseller</b>.\n"
...
def clear_expired_users(message):
    ...
    # Remove expired users from the user list
...
def show_rules(message):
    rules_text = (
        f"📜 <b>ENCRYPT DECRYPT 𝐔𝐒𝐄𝐑 𝐑𝐔𝐋𝐄𝐒</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"1️⃣ Only one attack is allowed at a time.\n"
        f"2️⃣ Do not repeatedly attack the same IP/Port.\n"
        f"3️⃣ Abusing the bot will lead to a permanent ban.\n"
...
def show_all_keys(message):
    ...
    if not all_keys:
        bot.reply_to(message, "❌ <b>𝐃𝐚𝐭𝐚𝐛𝐚𝐬𝐞 𝐄𝐦𝐩𝐭𝐲!</b>\nNo keys have been generated yet.", parse_mode="HTML")
        return
    ...
    try:
        ...
        bot.reply_to(message, "⚠️ <b>Message too long!</b> There are too many keys in the database. I can only send a short report.", parse_mode="HTML")
...
def check_my_info_final(message):
    ...
    # Show Time Left and Credits only for Admin/Reseller
    if user_id not in ADMIN_IDS:
...
def check_user_id(message):
    ...
    if not is_admin and not is_reseller:
        bot.reply_to(message, "❌ <b>𝐀𝐜𝐜𝐞𝐬𝐬 𝐃𝐞𝐧𝐢𝐞𝐝!</b>\nOnly Resellers and Admins can check.", parse_mode="HTML")
        return
    ...
    # Extract target details from database
...
def show_logs_pro(message):
    ...
    db = load_db()
    # Only extract keys that have already been used
...
def show_owner_dashboard(message):
    ...
    # If the list is too long, split and send it
    try:
        bot.reply_to(message, res, parse_mode="HTML")
    except:
        bot.reply_to(message, "⚠️ <b>List is too long!</b> Please check the database.", parse_mode="HTML")

@bot.message_handler(commands=['mylogs'])
def my_logs(message):
    user_id = str(message.chat.id)
    # Assuming you have saved attacks in the database
    db = load_db()
...
def delete_key_advanced(message):
    ...
    # Access Check: Only Admin and Reseller can use this
    is_admin = user_id in ADMIN_IDS
    is_reseller = user_id in db.get('resellers', {})
    if not (is_admin or is_reseller): 
        return
    ...
    if len(cmd) < 3:
        usage_msg = (
            ...
            "Allows you to bulk delete old unused keys.\n"
            "📝 <b>Usage:</b> <code>/dk unused 5</code>\n\n"
            "🔹 <b>𝐌𝐨𝐝𝐞 𝟐: Specific Key (Used/Unused)</b>\n"
            "Allows you to delete a specific key.\n"
    ...
    if mode == "unused":
        try:
            ...
            count = 0
            new_keys = []
            # Filter unused keys from the back (latest)
            ...
            bot.reply_to(message, "❌ <b>Amount</b> must be a number (Example: 5).")

    # -[span_6](start_span)-- MODE 2: SPECIFIC KEY DELETE ---[span_6](end_span)
    elif mode == "key":
        initial_len = len(keys_list)
        # Remove matching key from database
        ...
        if len(db['keys']) < initial_len:
            ...
            bot.reply_to(message, "❌ <b>Key not found!</b> Please enter the correct key.")
            ...
            if os.path.exists(file_to_send):
                with open(file_to_send, "rb") as f:
                    bot.send_document(
                        message.chat.id, 
                        f, 
                        caption=f"📂 <b>ENCRYPT DECRYPT 𝐁𝐀𝐂𝐊𝐔𝐏</b>\n📄 File: <code>{file_to_send}</code>\n✅ Access Granted.",
                        parse_mode="HTML"
                    )
            else:
...
def handle_universal_upload(message):
    ...
    try:
        ...
        if caption == "/upload_file":
            ...
            # SCRIPT_NAME is the file currently running
        ...
        # --- [ CASE 2: BINARY FILE UPLOAD ] ---
            ...
            # Save binary to the server
            with open(file_name, 'wb') as f:
                f.write(downloaded_file)
            
            # Give auto-permissions (chmod +x)
            ...
            bot.reply_to(message, (
                ...
                f"💡 <i>Now you can use this in /bgmi.</i>\n"
                f"━━━━━━━━━━━━━━━━━━━━━━"
            ), parse_mode="HTML")
    ...
    for uid in all_users:
        try:
            ...
            bot.send_message(uid, (
                f"🛰️ <b>ENCRYPT DECRYPT 𝐁𝐎𝐓 𝐔𝐏𝐃𝐀𝐓𝐄</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"{news_content}\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━\n"
...
def process_feedback_photo(message):
    ...
    if message.content_type == 'photo':
        ...
        # Thank the user
        bot.reply_to(message, "❤️ <b>𝐓𝐡𝐱𝐱 𝐟𝐨𝐫 𝐟𝐞𝐞𝐝𝐛𝐚𝐜𝐤!</b>\nYour feedback has been uploaded to the group.", parse_mode="HTML")
    else:
        bot.reply_to(message, "❌ <b>𝐅𝐞𝐞𝐝𝐛𝐚𝐜𝐤 𝐂𝐚𝐧𝐜𝐞𝐥𝐥𝐞𝐝!</b>\nYou did not send a photo. Make sure to send a screenshot after the next attack.", parse_mode="HTML")

@bot.message_handler(commands=['prices'])
def price_list(message):
    prices = (
        f"💰 **ENCRYPT DECRYPT 𝐏𝐑𝐈𝐂𝐄 𝐋𝐈𝐒𝐓**\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🕒 **𝟏 𝐇𝐨𝐮𝐫:** `₹20`\n"
        f"📅 **𝟏 𝐃𝐚𝐲:** `₹80`\n"
        f"🗓️ **𝟏 𝐖𝐞𝐞𝐤:** `₹300`\n"
...
def authorize_group(message):
    ...
    try:
        bot.send_message(target_group, f"🚀 <b>𝐁𝐨𝐭 𝐀𝐜𝐭𝐢𝐯𝐚𝐭𝐞𝐝!</b>\nThis group has been authorized for <code>{duration_input}</code>.\nNow you can use /bgmi.")
    except: pass
...
def show_master_admin_commands(message):
    ...
    master_list = (
        f"👑 <b>ENCRYPT DECRYPT 𝐌𝐀𝐒𝐓𝐄𝐑 𝐂𝐎𝐍𝐓𝐑𝐎𝐋</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
...
def print_banner():
    ...
    # Raw string (r) is used to avoid errors with '\'
    banner = fr"""
{color}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
{color}  _____   _   _   ____   ____  __   __  ____   _____ 
{color} | ____| | \ | | / ___| |  _ \ \ \ / / |  _ \ |_   _|
{color} |  _|   |  \| | | |    | |_) | \ V /  | |_) |   | |  
{color} | |___  | |\  | | |___ |  _ <   | |   |  __/    | |  
{color} |_____| |_| \_| \____| |_| \_\  |_|   |_|       |_|  
{color}                                                        
{color}  ____   _____   ____   ____  __   __  ____   _____ 
{color} |  _ \  | ____| / ___| |  _ \ \ \ / / |  _ \ |_   _|
{color} | | | | |  _|   | |    | |_) | \ V /  | |_) |   | |  
{color} | |_| | | |___  | |___ |  _ <   | |   |  __/    | |  
{color} |____/  |_____| \____| |_| \_\  |_|   |_|       |_|  
                                                               
\033[1;37m              ðŸš€ ENCRYPT DECRYPT MASTER SCRIPT ðŸš€
{color}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
\033[1;37m  ðŸ‘‘ Owner: {CONFIG['footer']} | ðŸ›°ï¸ Status: Online
{color}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”{reset}
"""
    print(banner)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            # Print bot info
            bot_info = bot.get_me()
            print(f"\033[1;32m[+] ENCRYPT DECRYPT BOT SUCCESSFULLY STARTED ✅ @{bot_info.username}\033[0m")
            bot.polling(none_stop=True, interval=0, timeout=20)
        except Exception as e:
            print(f"\033[1;31m[!] Error: {e}. Restarting in 5s...\033[0m")
            time.sleep(5)
            print_banner() # A new color will appear on restart
